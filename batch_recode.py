...
@click.option(
    "--crf",
    default=28,
    help="CRF (Constant Rate Factor) for x265. Allowed values in range 0-51. For more info please see https://slhck.info/video/2017/02/24/crf-guide.html",
)
@click.option(
    "--quality",
    default=35,
    help="Video quality for hevc_videotoolbox encoder. Allowed values in range 0-100.",
)
@click.option(
    "--encoder",
    type=click.Choice(
        ["libx265", "hevc_videotoolbox", "libsvtav1", "libaom-av1"],
        case_sensitive=False,
    ),
)
def main(
    directory, file_ext="mp4", recursive=False, crf=28, quality=35, encoder="libsvtav1"
):
    """Re-encode H264 video files in a directory using codecs (supported with --encoder option):
        - libx265
        - hevc_videotoolbox (Apple HW Acceleration)
        - libsvtav1 (AV1 option)
        - libaom-av1 (AV1 option)
    Args:
        ...
        quality: Similar to crf, but applicable to 'hevc_videotoolbox' encoding.
        encoder: One of 'libx265' | 'hevc_videotoolbox' | 'libsvtav1' | 'libaom-av1'
    """
    if recursive:
